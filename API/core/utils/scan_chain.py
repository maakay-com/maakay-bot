        ...
        if 'count' in r:
            if int(r['count']) > 0:
                ...
                if txs.direction == Transaction.INCOMING:
                    Statistic.objects.all().update(total_tnbc=F('total_tnbc') + txs.amount)
                else:
                    Statistic.objects.all().update(total_tnbc=F('total_tnbc') - txs.amount - settings.TNBC_TRANSACTION_FEE)
